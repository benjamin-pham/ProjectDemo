...
description: >
  ...
  Use this skill first when the user names a specific feature and asks why it's broken, how it
  works, or wants to change it — even if they don't say "handler". Trigger on: "the booking
  feature is broken", "how does the payment flow work", "why is GetBookings returning wrong data",
  "walk me through ReserveBooking", "which handler processes order cancellation",
  "debug the CreateOrder flow", "tính năng đặt phòng bị lỗi", "giải thích flow của X",
  "use case tạo order làm gì" — or any request about tracing, understanding, or fixing a
  named feature in a CQRS project.
  Do NOT trigger for general layer or structure questions without a specific named feature —
  use dotnet-clean-architect instead.
metadata:
  related-skills:
